        ...
        $regex = join('|',$regex);
  		...
  		case 'bigint':
			...
			case 'tinyint':
				...
				break;
			case 'smallint':
				$model_settings[$klass]['min_value'][$rec['Field']] = -32768;
				$model_settings[$klass]['max_value'][$rec['Field']] = 32767;
				$regex = '^\s*(\d+)\s*$';
				if ($model_settings[$klass]['is_auto_increment'][$rec['Field']] || $model_settings[$klass]['is_nullable'][$rec['Field']]) $regex = '(?:^\s*$)|'.$regex;
				$model_settings[$klass]['db_format'][$rec['Field']] = '/'.$regex.'/';
				$model_settings[$klass]['format'][$rec['Field']] = '/'.$regex.'/';
  ...
  foreach($tables as $table_name=>$fields)
  {
		...
		foreach($fields as $data)
		{
			...
			if (endswith($field_name, '_id'))
			{
			  ...
			  if ($data['Comment']!='')
			  {
			    $bt_table_name = trim($data['Comment']);
			  }
			  if($bt_table_name == '-') continue; 
		    if ( !array_key_exists( $bt_table_name, $tables ) )
		    {
		      Click::$meta['plugins']['clicklib']['settings']->activerecord->warnings[] = "Error in table $table_name.$field_name - mapping $bt_table_name in comment is not a valid table.";
		    }
...
$res = $wpdb->get_row("show variables where variable_name = 'ft_min_word_len'");
if ($res->Value > $activerecord_settings['ft_min_word_len']) click_error("mySQL FullText searching error. Set ft_min_word_len >= {$activerecord_settings['ft_min_word_len']}. Currently set to: ". $res[0]['Value']);
...
$recs = query_assoc("show tables");
$tables = collect($recs, "Tables_in_{$wpdb->dbname}");
foreach($tables as $k)
{
...
sort($keys);
$md5 = md5(join('|',$keys));
$settings->activerecord->codegen_hash=time();
